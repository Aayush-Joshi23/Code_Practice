class Solution {
    public int[] topKFrequent(int[] nums, int k) {
        ...
        for(int i=0; i<bucketPos.length; i++) {
            ...
            if(bucketPos[i] > 0) {
                if(bucketSorted[bucketPos[i]] == null) 
                    bucketSorted[bucketPos[i]] = new ArrayList<>();
                bucketSorted[bucketPos[i]].add(i);
            }

            if(bucketNeg[i] > 0) {
                if(bucketSorted[bucketNeg[i]] == null) 
                    bucketSorted[bucketNeg[i]] = new ArrayList<>();
